...
export class ContratoClausulaService {
  ...
  async findOne(id: number) : Promise<ContratoClausulas> {
    return await this.contratoClausulasRepository.findOne({where: {idContratoClausulas: id},relations:['contratos']});
  }
  ...
  async removeSeveral(id: number[]) : Promise<any> {
    const contratoClausulas = await this.contratoClausulasRepository.findBy({
      idContratoClausulas: In(id)
  });
    return await this.contratoClausulasRepository.remove(contratoClausulas);
  }
