...
export class AgenciasAseguradorasService {
  ...
  async findAll(): Promise<AgenciasAseguradoras[]> {
    return await this.agenciasAseguradorasRepository.find({relations:['contratos']});
  }

  async findOne(id: number) : Promise<AgenciasAseguradoras> {
    return await this.agenciasAseguradorasRepository.findOne(id,{relations:['contratos']});
  }
}
